...
class CTPManager:
    ...
    def create_md(self, front_address='tcp://180.168.146.187:10010', broker_id='9999'):
        ...
        print("交易日：", self.__md.get_market().GetTradingDay())  # 行情API中获取到的交易日
    ...
    def create_strategy(self, dict_arguments):
        ...
        self.__md.sub_market(list_instrument_id, user_id, strategy_id)  # 订阅行情
        for i in self.__list_user:
            if i.get_user_id().decode('utf-8') == dict_arguments['user_id']:
                self.__list_strategy.append(Strategy(dict_arguments, i))  # 创建策略实例
